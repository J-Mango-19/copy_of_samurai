...
def extract_number_before_extension(filename):
    # Regular expression to find the number before any file extension
...
def load_masks_from_dir(
    ...
    if not per_obj_png_file:
        ...
        if allow_missing and not os.path.exists(input_mask_path):
            return {}, None
        input_mask = load_ann_png(input_mask_path)
...
def save_masks_to_dir(
    ...
    if not per_obj_png_file:
        ...
        save_ann_png(output_mask_path, output_mask)
    else:
...
def vos_inference(
    predictor,
    base_video_dir, # directory that holds the video directory
    input_mask_dir,
    output_mask_dir,
    video_name,     # directory name that holds the images
    score_thresh=0.0,
    use_all_masks=False,
    ...
    for input_frame_idx in input_frame_inds:
        try:
            per_obj_input_mask = load_masks_from_dir(
    ...
    # write the output masks as PNG files to output_mask_dir
...
def main():
    ...
    for n_video, video_name in enumerate(video_names):
        ...
        if not args.track_object_appearing_later_in_video:
            ...
            raise NotImplementedError("I haven't converted this function from pallete PNGs to regular PNGs")
